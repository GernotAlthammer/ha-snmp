...
4. Pick which of the found printers to add (all are pre-selected). For each one, a device is created and populated automatically - no OIDs to type in:
   ...
   * One **Status** sensor per cover/door the printer reports (e.g. `Top Cover`, `Front Door`), shown as plain text (`Open`, `Closed`, `Interlock Open`, ...) - discovered by walking the printer's cover table (`prtCoverTable`), so it works for however many covers a given model has.
   * One **Level** sensor per paper tray the printer reports (e.g. `Tray 1`, `Tray 2`, `Manual Feed`), discovered by walking the printer's input-tray table (`prtInputTable`) - the same RFC 3805 sentinel values as toner levels (`-1`/`-2`/`-3`) are shown as text here too.
   * **Max-capacity** sensors (for toner/ink markers or paper trays) are left out by default to keep the entity list lean - add one manually afterwards (see below) if you need it, e.g. `1.3.6.1.2.1.43.11.1.1.8.1.<marker index>` for a marker or `1.3.6.1.2.1.43.8.2.1.9.1.<tray index>` for a tray.
